...
const PricingCard: React.FC<PricingCardProps> = ({ title, price, features, isPopular = false, buttonText }) => {
  ...
  const [priceAmount, priceUnit] = price.split(" ");

  return (
    <motion.div
      whileHover={{ scale: 1.02 }}
      transition={{ duration: 0.2 }}
      className={`p-8 rounded-xl border-2 relative hover:shadow-lg ${
        isPopular
          ? 'bg-gradient-to-br from-[#fff6ff] to-white border-[#6315e3]'
          : 'border-gray-200 bg-white'
      }`}
    >
      {isPopular && (
        <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
          <span className="inline-block px-4 py-1 text-sm font-medium rounded-full text-[#6315e3] bg-[#fff6ff]">
            Most Popular
          </span>
        </div>
      )}
      ...
      <div className="text-center mb-8">
        <h3 className="text-2xl font-bold mb-2 text-[#1f1926]">{title}</h3>
        <div className="flex items-center justify-center gap-2">
          <span className="text-4xl font-bold text-[#6315e3]">{priceAmount}</span>
          <span className="text-xl text-[#6315e3]">{priceUnit}</span>
      ...
      <motion.button
        ...
        className={`w-full py-3 rounded-full font-semibold ${
          isPopular
            ? 'bg-[#6315e3] text-white'
            : 'border-2 border-[#6315e3] text-[#6315e3]'
        }`}
      >
        {buttonText}
      </motion.button>
